...
object LineStreamSpecs extends Specification with ScalaCheck {
  ...
  "LineStream" should {
    ...
    "define equality by identity and not contents" in {
      val a = new LazyLineCons('a', sys.error("clever test failure"), "a", 1, 1, 0)
      val b = new LazyLineCons('a', sys.error("clever test failure"), "a", 1, 1, 0)
    ...
    "define hashCode by identity" in {
      new LazyLineCons('a', sys.error("clever test failure"), "a", 1, 1, 0).hashCode mustEqual 1
    }
    ...
    "define a different offset pair for each index" in {
      def allNums(ls: LineStream): Stream[Int] = {
        if (ls.isEmpty)
          Stream.empty
        else
          ls.offset #:: allNums(ls.tail)
      }
      ...
      check { str: String =>
        val ls = LineStream(str)
        Set(allNums(ls): _*) mustEqual (0 to str.length - 1).toSet
      }
    }
  }
